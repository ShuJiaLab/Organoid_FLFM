function s = z_integralParseArgs(varargin)
%INTEGRALPARSEARGS  Parse optional arguments to INTEGRAL.

%   Copyright 2007-2013 The MathWorks, Inc.

p = inputParser;
p.addParameter('AbsTol'     ,1e-10,@validateAbsTol     );
p.addParameter('RelTol'     ,1e-6, @validateRelTol     );
p.addParameter('Waypoints'  ,[],   @validateWaypoints  );
p.addParameter('ArrayValued',false,@validateArrayValued);
p.parse(varargin{:});

s = p.Results;
s.ArrayValued          = logical(s.ArrayValued);
s.Rule                 = z_Gauss7Kronrod15;
